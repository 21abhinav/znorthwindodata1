		...
		onTheMasterPage: {
			...
			actions: {
				...
				iSortTheListOnName: function () {
					return this.iChooseASorter("sortButton", "Sort By <ReportsTo>");
				},
				...
				iFilterTheListOnUnitNumber: function () {
					return this.iChooseASorter("filterButton", "<ReportsTo>", "<100 ");
				},

				iGroupTheList: function () {
					return this.iChooseASorter("groupButton", "<ReportsTo> Group");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor({
						...
						success: function (oList) {
							var aEntityData = this.getEntitySet("Employees");
							var aObjectsNotInTheList = aEntityData.filter(function (oObject) {
								return !oList.getItems().some(function (oListItem) {
									return oListItem.getBindingContext().getProperty("EmployeeID") === oObject.EmployeeID;
								});
							});
							var sObjectNotInTheListId;

							if (aObjectsNotInTheList.length) {
								sObjectNotInTheListId = aObjectsNotInTheList[0].EmployeeID;
							} else {
								// Not enough items all of them are displayed so we take the last one
								sObjectNotInTheListId = aEntityData[aEntityData.length - 1].EmployeeID;
							}
							...
							oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Employees", {
								EmployeeID: sObjectNotInTheListId
							});
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						title: oBindingContext.getProperty("ReportsTo")
					};
				}
			},
			...
			assertions: {
				...
				theListShouldBeFilteredOnUnitNumber: function () {
					return this.theListShouldBeFilteredOnFieldUsingComparator("ReportsTo", 100);
				},


				theListShouldBeSortedAscendingOnUnitNumber: function () {
					return this.theListShouldBeSortedAscendingOnField("ReportsTo");
				},
				...
				theListShouldHaveAllEntries: function () {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function (oList) {
							var aAllEntities = this.getEntitySet("Employees");
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							return this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "text",
									value: "<Employees> (" + iExpectedLength + ")"
								}),
								success: function () {
